...
(async () => {
    ...
    const handler = (dir) => {
        const handlerLoad = readdirSync("./handler/" + dir).filter(file => file.endsWith(".js"));
        for (const file of handlerLoad) {
            let eventName = file.match(/[^\\/]+(?=\.)(?!([^\\/]+[\\/]))/)[0];
            console.log(`Loading ${eventName}...`)
            require(`./handler/${dir}/${file}`)(client);
        }
    }
    await ["client", "music"].forEach(e => handler(e));
    client.handlerEvents(eventFiles);  
    client.handlerCommands(commandFiles);
})()
